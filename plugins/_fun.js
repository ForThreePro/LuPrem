let handler = async (m, { conn, command, text }) => {
    ...
    const BOX_TOP = `🧡━━「 🐾 **LU BOT PREM SYSTEM** 」━━🧡`
    const BOX_BOT = `╚━━「 **SOFA ACTIVO** 」━━╝`

    const frasesDuo = ["Duo de Lasaña 🍜","Juntos somos un peligro 😼","El duo que se roba la comida 💥","Duo de siesta nivel dios 😴","Dinamita con queso 🧨","El mejor duo del sofa 👑"]
    const frasesBro = ['"Oe mano pasame Lasaña"','"Ya pe no seas malo"','"Despues te pago juro"','"Invitame tu comida"']
    const frasesPerro = ['Te dice "amor" y a 3 gatos mas','Huele a croquetas ajenas','Te deja en visto por dormir','Come Lasaña sin ti']
...
╚━━「 **LU BOT PREM** 」━━╝`)
    ...
    switch(cmd) {
        ...
        case 'miamor':
            mentions = [target]
            txt = `${BOX_TOP}
║ 😼 **MODULO** : **AMOR DE LASAÑA**
║
║ 🐾 **TARGET** : ${jidToTag(target)}
║ 🍜 **NIVEL** : ${porcentaje}%
║ 💬 **DIAGNOSTICO** : ${porcentaje > 70? '**Almas Gemelas**' : porcentaje > 40? '**Hay Quimica**' : '**Frio Como Refri**'}
${BOX_BOT}`
            break
...
║
║ 🐾 **TARGET** : ${jidToTag(target)}
║ 🍜 **FRASE** : "**Te lo juro por mi Lasaña**"
║ 💬 **NIVEL** : ${porcentaje}%
${BOX_BOT}`
            break
        ...
        case '2p2': // 4 PERSONAS = 2 PAREJAS
            if(users.length < 4) return m.reply('🧡 **Minimo 4 michis en el grupo** 😼')
...
${BOX_BOT}`
        ...
        case '3p3': // 6 PERSONAS = 3 PAREJAS
            if(users.length < 6) return m.reply('🧡 **Minimo 6 michis en el grupo** 😼')
...
${BOX_BOT}`
        ...
        case 'duo': // 2 PERSONAS = 1 PAREJA
            if(users.length < 2) return m.reply('🧡 **Minimo 2 michis en el grupo** 😼')
